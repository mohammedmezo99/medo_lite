...
if [[ "$rom_os" != "OS2" ]]; then
    warn "[OS2 MIUI Launcher] skipped: ROM is not OS2"
    exit 0
fi
...
cp -rf "$work_dir/bin/modfile/OS2/launcher/MiuiHome" "$MAIN_FOLDER/product/priv-app/"
cp -rf "$work_dir/bin/modfile/OS2/launcher/XiaomiEUExt" "$MAIN_FOLDER/product/priv-app/"
cp -rf "$work_dir/bin/modfile/OS2/launcher/permissions/." "$MAIN_FOLDER/product/etc/permissions/"
